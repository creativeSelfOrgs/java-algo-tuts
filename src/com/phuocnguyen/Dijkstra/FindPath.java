package com.phuocnguyen.Dijkstra;
...
public class FindPath {
	...
	private final String SET_NAME_TO_FILE_INITIAL = "Input1-51503082-NguyenHuuPhuoc.txt";
	private final String SET_NAME_TO_RESULT = "LS_10-51503082-NguyenHuuPhuoc_Nhom2.txt";
	...
	private Scanner in;
	private Scanner in1;
	private Scanner in12;
	private Scanner in13;
	private Scanner in14;

	public int nodeid(int in) {
		return (in + 1);
	}

	public void generateValues() throws IOException {
		BufferedWriter bufferedWriter = null;
		FileWriter fileWriter = null;
		Random random = new Random();
		fileWriter = new FileWriter(SET_NAME_TO_FILE_INITIAL);
	...
	/* MENU */
	public void callDijkstra() throws IOException {
		...
		while (true) {
			...
			if (regmessage.equals("1")) {
				...
				for (int n = 0; n < Max_rooters; n++) { // EXTRA FEATURE IMPLEMENTATION -> TO DISPLAY CONNECTION TABLE
														// FOR ALL NODES
					source = n;
					...
					if (DistGrph[source][source] == 0) {
						if ((destination > -1) && (destination < Max_rooters)) {
							if (DistGrph[destination][destination] == 0) {
								...
								System.out.println("Destination Rooter is Down");
							}
						} else {
							System.out.println("Destination node is not selected");
						}
					} else {
						System.out.println("Source Rooter is Down");
					} // Router Check conditions

				} else {
					System.out.println("Source node is not selected");
				}
			}
	...
	public void ComputeConnectionTabel() {
		...
		if (DistGrph[source][source] == 0) {
				...
				for (int k = 0; k < Max_rooters; k++) {
					if (conTable[k].flg) {
						if (DistGrph[tmpsorce][k] != -1) {
							...
							} else { // selected node length is added to length table entry for new length
								conTable[k].length = conTable[tmpsorce].length + DistGrph[tmpsorce][k];
			...
			for (int i = 0; i < Max_rooters; i++) {
				...
				if (conTable[i].ids[1] == -1) {
					tmp = "-1"; // Check the Router ID if it is -1
				}
				if (i == source) {
					tmp = "-"; // Source to Source Router will be "-"
				}
	...
	public void PrintConnectionTabel() {
		...
		String str_source = in12.nextLine();
	...
	public void PrintShortPathToDestination() throws IOException {
		BufferedWriter bufferedWriter = null;
		FileWriter fileWriter = null;
		fileWriter = new FileWriter(SET_NAME_TO_RESULT);
		bufferedWriter = new BufferedWriter(fileWriter);
		System.out.println("Enter Destination Rooter Id< 1 - " + (Max_rooters) + " >:");
		in13 = new Scanner(System.in);
		String str_dest = in13.nextLine();
		...
		if (DistGrph[source][source] == 0) {
			if (DistGrph[destination][destination] == 0) {
				...
				System.out.print(
						"Shortest Path from Rooter:[" + nodeid(source) + "] to [" + nodeid(destination) + "] is: ");
				...
				if (conTable[destination].length > 0) {
					...
					System.out.println();
					System.out.println("The total cost is " + conTable[destination].length);
					bufferedWriter.write(String.valueOf(conTable[destination].length));
				} else {
					System.out.println("Path Not Available");
					bufferedWriter.write("Path Not Available");
				}
			} else {
				System.out.println("Destination Rooter is Down"); // If Destination Router is down
				bufferedWriter.write("Destination Rooter is Down");
			}
		} else {
			System.out.println("Source Rooter is Down"); // If Source Rooter is down
			bufferedWriter.write("Source Rooter is Down");
		}
		bufferedWriter.close();
		fileWriter.close();
	...
	public void ReadTextfileToBuildGraph() {
		try {
			...
			in14 = new Scanner(System.in);
			String fiename = in14.nextLine();
